...
SELECT useraccounts.accountNo, useraccounts.cnic, transaction.action, transaction.transactionId FROM useraccounts INNER JOIN transaction ON useraccounts.id=transaction.userId WHERE useraccounts.id=2;
...
-- Retrieve the count of number of transactions made by the account holder with the user Id 1
SELECT COUNT(action) AS TotalTransactions FROM transaction WHERE transaction.userId=1;

-- Find out the difference between total debit transaction amounts and total credit transaction amounts which 
-- has happened in the bank 

SELECT (SUM(debit) SUM(credit)) AS Debit CreditDiff FROM transaction;

-- Find the maximum balance which is there in any the accounts in the bank
SELECT MAX(balance) FROM useraccounts;

--Set Operations 
-- Retrieve the name, account number and balance of those accounts whose 
-- accounts have a balance either less than 10000 or more than 20000. 
(SELECT feedbackId, message, userId, date FROM feedback) EXCEPT (SELECT feedbackId, message, userId, date FROM feedback WHERE userId =1);

-- Retrieve all the feedbacks sent to the bank except for the ones sent by customer 
-- with user ID = 1 
(SELECT feedbackId, message, userId, date FROM feedback)
EXCEPT
(SELECT feedbackId, message, userId, date FROM feedback WHERE userId = 1);
